...
@export var PJ1 : Node3D
@export var PJ2 : Node3D
...
@export var min_topdownview_deg : float = deg_to_rad(-10.0)
...
func pjs_side():
	CamtoCenter_vector = Vector3(Cam_Virtual_Pos.x, 0.0, Cam_Virtual_Pos.z).normalized()
	PJ1Cam_vector = Vector3(PJ1_Pos.x-MidPoint_PJ.x, 0.0, PJ1_Pos.y-MidPoint_PJ.y).normalized()
	
	PJ1_crossed = CamtoCenter_vector.cross(PJ1Cam_vector)
